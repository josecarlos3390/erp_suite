...
- **Las ciudades publican sus tiendas, en las dos modalidades que el ERP puede configurar (2026-09-25, T235 — Fase 2 #2/A2b)**. El canal publica ahora, dentro de cada ciudad, sus **puntos de venta/retiro**: los configura la empresa en el ERP y pueden colgar de una **sucursal** (una sucursal por tienda) o de un **almacén** que hace de tienda (la empresa que solo crea la sucursal principal y pone sus locales como almacenes). Lo importante para la tienda es que **la forma de la respuesta es la misma** en los dos casos —el `kind` dice de qué cuelga y los datos llegan **ya resueltos**, con la herencia del maestro aplicada—, así que **el front no ramifica**. **Entregado**: `/sucursales` pinta bajo cada ciudad la lista **«Tiendas en …»** (`city-stores`), con cada tienda (`city-store`) y su dirección, horario, teléfono (`tel:`) y «Ver el mapa» (`store-map`), más la insignia `store-pickup` (**«Retiro disponible»** / **«Solo venta»**) según lo que la empresa haya habilitado. **Medido**: `e2e:visual` **17/17**, `e2e:a11y` **19/19**, `e2e:perf` **5/5**, `typecheck` 0, `lint` 0 avisos y `next build` 0 (14/14 páginas; `/sucursales` 181 B). **Declarado**: `City.stores` es **opcional** en el tipo a propósito — el fixture grabado del gate visual es anterior a esta clave y los tres gates siguen verdes **sin regrabarlo** (la lección de T223) —; `/sucursales` todavía no entra en el barrido de axe ni en las capturas del gate visual, y el **selector de tienda en el checkout** (elegir dónde retirar) es el tramo siguiente (C), que además llevará la tienda elegida al pedido.
