...
test.describe("Ficha de producto", () => {
  test("muestra precio, disponibilidad de la ciudad y el JSON-LD de Product/Offer", async ({
    ...
    expect(normalize(priceText ?? "")).toBe(
      expectedMoney(detail.price, detail.currency),
    );

    // F6: la ficha dice quien vende (el vendedor que publica el canal, no un texto propio).
    if (detail.seller !== null) {
      await expect(page.getByTestId("product-seller")).toContainText(
        detail.seller,
      );
    } else {
      // Sin vendedor asignado la tienda no inventa uno: no hay linea.
      await expect(page.getByTestId("product-seller")).toHaveCount(0);
    }
    ...
    if (detail.specs.length > 0) {
      ...
      await expect(
        page.locator('[data-testid="specs-table"] tbody tr'),
      ).toHaveCount(detail.specs.length);
  ...
  test("un slug inexistente responde 404 con la pagina de la tienda", async ({
    page,
  }) => {
