        ...
        <div className="flex flex-col gap-4 lg:sticky lg:top-32">
          ...
          {product.badges.length > 0 ? (
            <ul className="flex flex-wrap gap-2">
              {product.badges.map((badge) => (
                <li key={badge}>
                  <Badge variant={badge.trim().toUpperCase() === 'OFERTA' ? 'promo' : 'soft'} testId="product-badge">
                    {badge}
                  </Badge>
                </li>
              ))}
          ...
          <section className="sf-panel flex flex-col gap-2" aria-label="Precio">
            <div className="flex flex-wrap items-baseline gap-3">
              ...
              {discount !== null ? (
                <Badge variant="deal" testId="detail-discount-badge" srLabel={`Descuento de ${discount}`}>
                  {discount}
                </Badge>
              ) : null}
              {promoPct !== null ? (
            ...
            {compareAt !== null ? (
              <p className="text-xs text-fg-secondary">
                Antes <span className="sf-price-compare">{formatMoney(compareAt, product.currency)}</span> ·
                ahorras {formatMoney(compareAt - product.price, product.currency)}
              </p>
            ) : null}
          ...
          <ul className="grid gap-2 sm:grid-cols-2">
            <li className="sf-panel flex items-start gap-2 p-3">
              ...
              <span className="flex flex-col">
                ...
                <span className="text-2xs text-fg-tertiary" data-testid="product-city">
                  Ciudad de entrega: {city.name}
                  {product.availability.warehouseId !== null ? ` (almacen ${product.availability.warehouseId})` : ''}
                </span>
              </span>
            </li>
            ...
            <li className="sf-panel flex items-start gap-2 p-3">
              ...
              <span className="flex flex-col">
                ...
                <span className="text-2xs text-fg-secondary">
                  {describeShipping(city.shippingCost, city.freeShippingFrom, product.currency)}
                </span>
                <span className="text-2xs text-fg-tertiary">Se cobra una sola vez por pedido.</span>
              </span>
            </li>
            ...
            <li className="sf-panel flex items-start gap-2 p-3">
              ...
              <span className="flex flex-col">
                ...
                <span className="text-2xs text-fg-tertiary">
                  <Link href="/paginas/envios-y-devoluciones" className="sf-link">
          ...
          {product.shortDescription !== null ? (
            <p className="text-sm leading-relaxed text-fg-secondary">{product.shortDescription}</p>
          ) : null}
        </div>
      </div>

      <section aria-labelledby="ficha-titulo" className="sf-section">
        <SectionHeader id="ficha-titulo" eyebrow="Detalle" title="Ficha tecnica" />
        {specGroups.length === 0 ? (
          ...
          <div className="flex flex-col gap-2">
            {specGroups.map((group) => (
              <details key={group.group} className="sf-card overflow-hidden" open>
                <summary className="flex cursor-pointer items-center justify-between gap-2 p-4 text-sm font-semibold text-fg">
                  ...
                  <span className="text-2xs font-normal text-fg-tertiary">
                    {group.rows.length} {group.rows.length === 1 ? 'caracteristica' : 'caracteristicas'}
                  </span>
                </summary>
                <table className="w-full border-collapse text-sm" data-testid="specs-table">
                  ...
                  <tbody>
                    {group.rows.map((spec) => (
                      <tr key={`${group.group}-${spec.name}`} className="border-t border-line-subtle">
                        <th scope="row" className="w-1/2 px-4 py-2 text-left font-medium text-fg-secondary">
                          {spec.name}
                        </th>
