 ...
 *  - Antes de `next start` se **vacia la cache de datos de Next**
 *    (`e2e/visual/reset-data-cache.mjs`, T230): esa cache vive en disco y sobrevive al
 *    arranque, asi que sin vaciarla la primera visita de cada corrida se pintaba con
 *    los datos de la corrida anterior (medido: `producto-claro` salia sin el bloque de
 *    servicios ni el formulario de servicio tecnico mientras `producto-oscuro` si los
 *    pintaba).
 *
...
export default defineConfig({
  ...
  webServer: [
    ...
    {
      // La caché de datos de Next **vive en disco** y sobrevive al `next start`, así que
      // la primera visita a una ruta de cada corrida se pintaría con los datos de la
      // corrida anterior (`stale-while-revalidate`). Se vacía antes de arrancar: el
      // defecto, la medición y el porqué están en `e2e/visual/reset-data-cache.mjs`.
      command: `node e2e/visual/reset-data-cache.mjs && npx next start -p ${PORT}`,
