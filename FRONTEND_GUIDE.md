...
| `erp-frontend/src/styles/CSS-ARCHITECTURE.md` | **Arquitectura CSS y política de especificidad**: capas, orden de emisión de los parciales, receta para ganar sin `!important` (repetir la clase propia), casos legítimos de `!important`, política de `::ng-deep`, gates de densidad y método de verificación visual. | **Antes de escribir o modificar SCSS global** (`src/styles/**`, `styles.scss`) o al resolver una pelea de especificidad con un componente LUNA. |
