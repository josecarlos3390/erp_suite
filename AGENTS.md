...
> **Última actualización:** 2026-09-18 (recordatorio de devolución pendiente NC→Entrega/Recepción de punta a punta —T144—: utilidad derivada compartida, badge en listados, panel en la NC, regla R14f del detector y E2E de backend + UI; declarados T145 y T146; matriz de flujos en `docs/reference/matriz-flujos-documentos.md`).
...
npm test                 # jest — 171 suites / 1964 tests
npm run test:watch       # jest --watch
npm run test:cov         # jest --coverage
npm run test:e2e         # jest E2E — 18 suites / 122 tests (sincroniza antes la BD de tests)
...
| `npm test` | ✅ **OK** | **171 suites / 1964 tests passed** (2026-09-18, con los **13 unitarios del pendiente de devolución** —T144, `src/common/pending-return.util.ts`— sobre los 170/1951 de G1, que sumó los 24 del reparto del Precio de Entrega sobre los 169/1927 de T136, que sumó 8 sobre los 168/1919 de T132; antes, los 8 tests nuevos de **T135/T136** —4 del emparejamiento por rondas compartido con las sugerencias y 4 de la valoración del almacén en el seed— sobre los 168/1919 de T132, que a su vez sumó 15 sobre los 166/1904 de T131; el hook de pre-push lo corre completo en verde contra `origin` y `deploy`) |
| `npx tsc --noEmit` (proyecto y specs) | ✅ **OK** | 0 errores |
| `npm run test:e2e` | ✅ **OK** | **18 suites / 122 tests passed** (2026-09-18, con la suite nueva de **T144** `test/credit-note-return-pending.e2e-spec.ts` **4/4** —pendiente derivado en la NC y en el listado, vínculo completo de la línea, FRV acreditada antes de entregar → pendiente 0, espejo de compras y aislamiento— y la de **G1** `test/landed-costs.e2e-spec.ts` **7/7**; el script sincroniza antes el esquema de `erp_test` con `prisma db push` — ver nota de entorno). **Aviso de entorno medido el 2026-09-17**: `test/app.e2e-spec.ts` falló una vez con `disk: down` porque el disco estaba al **90,3 %** (la caché `.angular` del frontend había crecido a **44,4 GB**, ver T142); ese chequeo depende de `HEALTH_DISK_THRESHOLD_PERCENT`, es decir del **estado del disco de la máquina**, no del código — liberada la caché la suite pasa **118/118**. **Franja horaria (T146, declarado)**: en la ronda de T144 la suite cerró **1 failed / 121 passed** en `withholding-reports.e2e-spec.ts` (`ledgerTotal` esperado 0, recibido 33.9) y el mismo test **pasa 6/6** al cruzar la medianoche local: es la clase de **T134** (el «hoy» del suite en UTC frente al día del **tenant**, UTC−4) en un reporte del backend — ver AUDIT T146 |
...
> - **NC → Devolución: recordatorio derivado y dos defectos declarados (T144–T146, 2026-09-18).** La devolución **nace de la entrega/recepción** (su menú «Copiar a»), no de la nota de crédito, así que una NC puede dejar mercancía **acreditada sin devolver**: el ERP lo **deriva** (`src/common/pending-return.util.ts`: `min(acreditado, entregado/recibido) − devuelto`, anclado a la línea de entrega y nunca desde `invoicedQty`) y lo publica en el listado de entregas/recepciones (badge «NC sin devolver»), en la NC (`GET /<nc>/:id/return-pending` con **un enlace por documento pendiente** al flujo de devolución que ya existe) y en la regla **R14f** del detector, **como aviso**. Al construirlo apareció que la NC creada **por API** guardaba el vínculo a medias (`baseLineId` sin `baseDocType`/`baseDocId`), lo que hacía que **R3** reportara un falso ERROR de `invoicedQty`: el servicio **deriva** ahora el vínculo de la cabecera (ventas y compras). **Declarados y ABIERTOS** (con su reproducción en AUDIT): **T145** —una NC **parcial** de una línea con importes impares descuadra el asiento un céntimo y responde 500, por derivar el neto como producto flotante y el IVA por separado— y **T146** —el E2E de retenciones sufridas falla en la franja 20:00–24:00 locales por el desfase de día UTC/tenant (clase T134)—.
