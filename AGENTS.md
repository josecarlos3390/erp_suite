...
> **Última actualización:** 2026-09-18 (T145 cerrado —la NC parcial de importes impares ya cuadra al céntimo: canon de dinero en la derivación y en los totales—, T146 cerrado por su causa real —el arnés E2E fechaba con el día UTC y pedía documentos del futuro del tenant; ahora usa `tenantToday()`— , T147 declarado (la reversa se fecha con el «hoy» del tenant) y T148 registrado (el `isReserve` por defecto del API se mantiene en `'Y'`); matriz de flujos en `docs/reference/matriz-flujos-documentos.md`).
...
npm test                 # jest — 171 suites / 1973 tests
npm run test:watch       # jest --watch
npm run test:cov         # jest --coverage
npm run test:e2e         # jest E2E — 20 suites / 128 tests (sincroniza antes la BD de tests)
...
npm test                 # Karma + Jasmine — 1680 tests
npm run e2e              # playwright test — suite completa (incluye capturas y diagnósticos)
npm run e2e:functional   # gate funcional de escritorio (228 tests) en una sola pasada — config propia
...
| `npm test` | ✅ **OK** | **171 suites / 1973 tests passed** (2026-09-18, con los **9 unitarios de T145/T146** —6 del prorrateo de la línea de factura de la NC, con el invariante `neto + IVA === total` y el empate de medio centavo, y 3 de `tenantToday` con el borde 20:00–24:00 locales— y los **13 del pendiente de devolución** —T144, `src/common/pending-return.util.ts`— sobre los 170/1951 de G1, que sumó los 24 del reparto del Precio de Entrega sobre los 169/1927 de T136, que sumó 8 sobre los 168/1919 de T132; antes, los 8 tests nuevos de **T135/T136** —4 del emparejamiento por rondas compartido con las sugerencias y 4 de la valoración del almacén en el seed— sobre los 168/1919 de T132, que a su vez sumó 15 sobre los 166/1904 de T131; el hook de pre-push lo corre completo en verde contra `origin` y `deploy`. **Nota de entorno**: una corrida lanzada **en paralelo con el gate de UI de Playwright** cerró `1 failed / 1972 passed` sin que se capturara el nombre del test; las **dos corridas siguientes en solitario dieron 1973/1973**, y ninguna suite unitaria abre la BD real (todas mockean Prisma), así que se atribuye a la contención de CPU de esa corrida concurrente) |
| `npx tsc --noEmit` (proyecto y specs) | ✅ **OK** | 0 errores |
| `npm run test:e2e` | ✅ **OK** | **20 suites / 128 tests passed** (2026-09-18, con las suites nuevas de **T145** `test/credit-note-partial-money.e2e-spec.ts` **3/3** —dinero de la NC parcial con importes impares, ventas y compras, con el asiento cuadrado— y del **default `isReserve`** `test/sale-invoice-from-delivery-default.e2e-spec.ts` **2/2** —decisión de contrato del punto 3, con el stock intacto en los dos casos—, más la de **T144** `test/credit-note-return-pending.e2e-spec.ts` **4/4** —pendiente derivado en la NC y en el listado, vínculo completo de la línea, FRV acreditada antes de entregar → pendiente 0, espejo de compras y aislamiento—, la del **límite declarado (T147)** en `test/withholding-reports.e2e-spec.ts` y la de **G1** `test/landed-costs.e2e-spec.ts` **7/7**; el script sincroniza antes el esquema de `erp_test` con `prisma db push` — ver nota de entorno). **Aviso de entorno medido el 2026-09-17**: `test/app.e2e-spec.ts` falló una vez con `disk: down` porque el disco estaba al **90,3 %** (la caché `.angular` del frontend había crecido a **44,4 GB**, ver T142); ese chequeo depende de `HEALTH_DISK_THRESHOLD_PERCENT`, es decir del **estado del disco de la máquina**, no del código — liberada la caché la suite pasa **118/118**. **Franja horaria (T146, CERRADO)**: en la ronda de T144 la suite cerró **1 failed / 121 passed** en `withholding-reports.e2e-spec.ts` (`ledgerTotal` esperado 0, recibido 33.9) solo entre las 20:00 y las 24:00 locales; la causa medida fue el **arnés** —fechaba con el día **UTC** (`toISOString()`) y pedía documentos del **futuro** del tenant (UTC−4), así que el asiento original caía en el día siguiente y su reversa, fechada con el «hoy» del tenant, quedaba fuera de la ventana del informe— y el arreglo es `tenantToday()` (113 usos en 13 specs) — ver AUDIT T146 y el límite real en T147 |
...
1. **Gaps de producto priorizados (2026-09-08)** — ver `docs/plans/plan-gaps-deuda-2026-09.md`:
   ...
   `docs/plans/plan-g1-precio-entrega.md` y AUDIT T142/T143; quedan **G2 revalorización de
   artículos** (comparte el motor de costos), producción (potenciar ensamblaje) y servicios
   (OT/contratos). Módulos contables que AGENTS listaba como pendientes (cierre de período,
