...
> **Última actualización:** 2026-09-17 (G1 «Precio de Entrega» / costeo de importación implementado de punta a punta: backend + frontend + E2E de UI + los tres sitios de limpieza global —T142/T143—; matriz de flujos en `docs/reference/matriz-flujos-documentos.md`).
...
npm test                 # jest — 170 suites / 1951 tests
npm run test:watch       # jest --watch
npm run test:cov         # jest --coverage
npm run test:e2e         # jest E2E — 17 suites / 118 tests (sincroniza antes la BD de tests)
...
npm test                 # Karma + Jasmine — 1670 tests
npm run e2e              # playwright test — suite completa (incluye capturas y diagnósticos)
npm run e2e:functional   # gate funcional de escritorio (225 tests) en una sola pasada — config propia
...
| `npm test` | ✅ **OK** | **170 suites / 1951 tests passed** (2026-09-17, con los **24 unitarios del reparto del Precio de Entrega** —G1, `src/common/landed-cost.util.ts`— sobre los 169/1927 de T136, que sumó 8 sobre los 168/1919 de T132; antes, los 8 tests nuevos de **T135/T136** —4 del emparejamiento por rondas compartido con las sugerencias y 4 de la valoración del almacén en el seed— sobre los 168/1919 de T132, que a su vez sumó 15 sobre los 166/1904 de T131; el hook de pre-push lo corre completo en verde contra `origin` y `deploy`) |
| `npx tsc --noEmit` (proyecto y specs) | ✅ **OK** | 0 errores |
| `npm run test:e2e` | ✅ **OK** | **17 suites / 118 tests passed** (2026-09-17, con la suite nueva de **G1** `test/landed-costs.e2e-spec.ts` **7/7**; el script sincroniza antes el esquema de `erp_test` con `prisma db push` — ver nota de entorno). **Aviso de entorno medido el 2026-09-17**: `test/app.e2e-spec.ts` falló una vez con `disk: down` porque el disco estaba al **90,3 %** (la caché `.angular` del frontend había crecido a **44,4 GB**, ver T142); ese chequeo depende de `HEALTH_DISK_THRESHOLD_PERCENT`, es decir del **estado del disco de la máquina**, no del código — liberada la caché la suite pasa **118/118** |
