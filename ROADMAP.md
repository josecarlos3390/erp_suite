...
|---|---|---|
| **G3 — Módulo de Producción (órdenes, emisión/recibo, WIP)** | plan-gaps 2026-09 (grupo 1) | 🚧 **Fases 1, 2 y 3 implementadas (2026-09-19)**, plan aprobado por el usuario: plan `docs/plans/plan-g3-produccion.md` y estudio comparativo `docs/reference/PRODUCCION_ERP_COMPARATIVA.md` (cómo lo resuelven SAP B1, Odoo 19 y Dynamics 365, verificado contra documentación pública). Alcance propuesto: maestros de **centro de trabajo / recurso / componente de costo** (con las cuentas contables parametrizables que pidió el usuario), **BOM multinivel** con merma y operaciones, **orden de producción** con explosión de BOM y aviso de faltantes, **emisión para producción** (Dr WIP · Cr Inventario), consumo de recursos, **recibo de producción** (Dr PT · Cr WIP) con coproducto y desperdicio, y **cierre con desviación** que deja el **WIP en 0** (flujo SAP B1, con estados reversibles). Decisiones aprobadas por el usuario: D1 órdenes con explosión de BOM y aviso de faltantes (**sin MRP ni APS**), D2 **costo real por orden con liquidación al cierre** (sin costo estándar), D3 recurso con componentes de costo y **máquina = recurso `MACHINE` enlazado al Activo Fijo**, D4 escribir la referencia y el plan **antes** de codificar. Reutiliza el motor contable por jerarquía, el kardex, las series, las dimensiones/proyectos, la guarda de período (T151) y el motivo de anulación (T153); **`AssemblyOrder` no se migra** (conserva su contabilidad sin WIP). Límites declarados: sin MRP/APS, sin nómina (la mano de obra entra por componentes de costo a una tarifa), sin costeo estándar ni centro de costo en la línea de asiento (se resuelve por proyecto/dimensiones/regla de distribución) |
