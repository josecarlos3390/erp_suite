...
| **Lógica de licencias sobre los roles** (plan/subscripción → roles/features habilitadas) | ROADMAP 3.6 / Fase 8.1 | ✅ Infraestructura + **matriz de producto aprobada** (2026-09-09, T46): matriz `2026-09-09.2` con módulos core como piso común (ambos planes) y capacidades del grupo 2 con destino por plan (SIN/CRM/divisa en ambos; SAP, nómina avanzada y localización solo DEDICATED; `planned` para las no implementadas) + `planIncludes` + `GET /billing/capabilities`. Sin gating de módulos actuales (D1 no regresión); volumetría por plan diferida (D3). Propuesta en `docs/plans/propuesta-matriz-licencias-2026-09.md`. Ver AUDIT T46 |
