...
| Media | **F5.6 — Reporte y certificado de «retenciones sufridas» (ventas)** | El asiento y la validación de la retención en el cobro ya existen (T91), pero **no hay reporte ni certificado**: hoy hay que reconstruirlo leyendo el mayor. Alcance y **criterio de aceptación**: (a) listado por **período, cliente y tipo de retención** con base, porcentaje, importe retenido, documento y fecha del cobro; (b) **totales por tipo** y por cliente; (c) exportación (PDF con formato de certificado + Excel); (d) **cuadre**: la suma del reporte por tipo y período debe **igualar el movimiento del mayor** de la cuenta de retenciones correspondiente (mismo criterio de centavo que el resto del ERP), verificado con un E2E que compare reporte ↔ asiento; (e) permiso propio y filtro por sucursal. Origen del riesgo: se declaraba como «riesgo de QA» y en realidad es **feature de producto** (ver `AUDIT.md` §8 T126, nota de reclasificación). |
