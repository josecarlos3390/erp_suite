...
| **Cuentas contables editables en las líneas (frente nuevo, 2026-09-21)** | petición del usuario (T163) | 🟡 **P1 hecha · P2–P5 pendientes de aprobación**: la cuenta se **ve** en la línea, se **propone** desde la determinación y se puede **cambiar a mano**. Hecho: **Revalorización** (cuentas de aumento y contrapartida por línea, con validación de detalle/activo/tenant antes de la transacción y preferencia de la capturada en el asiento). Pendiente: **P2** inventario (el motor ya respeta `acctCode`; falta la columna en las 4 grillas), **P3** comerciales (facturas, F. Reserva, NC/ND, entregas, recepciones), **P4** familias sin *override* (producción, precio de entrega, ensamblaje, activos fijos, tomas de inventario) y **P5** guardarraíl de CI. Plan: `docs/plans/plan-cuentas-contables-editables.md`; evidencia de P1 en AUDIT **T163** |
| **Centros de costo, dimensiones y normas de reparto (frente nuevo, 2026-09-21)** | petición del usuario | 🟡 **Plan propuesto, decisión de diseño pendiente**: hoy los **5 ejes** (`DimensionConfig`) nombran las dimensiones, el **centro de costo es un valor de un eje** (`CostCenter.dimensionNumber`, como SAP B1) y las **normas de reparto** (`DistributionRule` + líneas por porcentaje) **ya se aplican al asiento contable manual** (expanden la línea en una por centro de costo con el importe prorrateado, escribiendo el código del centro en la dimensión del eje); lo que falta es (a) capturarlos también en las líneas de los **documentos**, (b) aplicar el reparto a los asientos **generados por documentos** y (c) los **informes** por centro de costo y por norma. Recomendación: **reutilizar el eje** (sin columna nueva) + validar el valor contra el maestro + extender el reparto a los documentos + los dos informes. Plan con las tres opciones y las preguntas abiertas: `docs/plans/plan-centros-costo-y-reparto.md` |
