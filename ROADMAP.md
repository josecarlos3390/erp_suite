...
| **Gate visual `e2e:visual` con deriva de ±2px** en formularios de documentos/pagos (subconjunto rotativo entre corridas) | AUDIT T48/T51 (QA E2E) | ⏳ **Abierto (2026-09-09, T53)**: no es regresión de código (falla igual con `src/` en el commit del baseline `b307cac4`, con la BD recién seedeada y con la pasada de calidad en stash); la divergencia es siempre ±2px de altura en la fila de líneas y depende de las tipografías web (Google Fonts) + estado asíncrono. Acciones: espera explícita de webfonts en `gotoForm` y 4 baselines de stock regenerados → `e2e:visual` 48/52. **Siguiente paso**: self-hostear las fuentes (`src/assets/fonts`) o normalizar el alto de las capturas. Ver AUDIT T53 |
