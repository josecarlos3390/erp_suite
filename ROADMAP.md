...
| ✅ DT.4 | **Colores hardcodeados** | **CERRADO (2026-09-16, T133)** con **gate propio** `npm run audit:tokens`: hex fuera de la capa de tokens (`styles.scss` + `styles/tokens/**`) = 0 sin justificar (18 literales declarados con `// color-ok: <razón>`, todos acentos/constantes de tema sin token). En el cierre se retiraron **68** fallbacks muertos `var(--token, #hex)`, se corrigieron **9** fallbacks **vivos** (el token NO existía → el color no seguía el tema oscuro: `--luna-*`, `--surface-raised`, `--color-neutral` —este último **se creó**—, `--primary-faint`) y se migraron a su token los pares claro/oscuro exactos (`--color-primary-text`, `--color-warning-text`, `--status-warning-*`, `--color-warning`, `--text-on-primary`, `--text-on-danger`). Se eliminó CSS muerto (`.warning-box-sm`). Ver `AUDIT.md` T133 | ✅ CERRADO |
| ✅ DT.5 | **Selectores nativos** | **CERRADO (2026-09-16, T133)**: **0 `<select>` nativos** en plantillas de `src/` (los 5 que quedaban, en el **constructor de consultas**, pasaron a `luna-select` con `[options]`; los campos no operables viajan `disabled` como antes) y el editor entra por primera vez en los **baselines visuales** (`readySelector` propio) para no volver a migrarlo sin red. Karma del editor 9/9 (1 test nuevo por estilo/opciones) | ✅ CERRADO |
| ✅ DT.6 | **Responsive / touch targets** | **CERRADO (2026-09-16, T133)**: las **84** media queries con px crudo pasaron a los breakpoints canónicos (`bp.$breakpoint-xs/sm/md/lg/xl`, con `$breakpoint-lg-up` para el complemento `min-width`, y el one-off de 720px unificado a `lg`); el gate `audit:tokens` (R2) falla ante cualquier `@media` con px nuevo. Los touch targets quedan cubiertos por la escala de tamaños (`--size-control-xs/sm/md/lg`) —los dos micro-botones de 26px de `batch`/`serial-selector` pasaron al token `xs`— y por el gate móvil `npm run e2e:mobile` | ✅ CERRADO |
...
| ~~Media~~ | ~~**F5.6 — Reporte y certificado de «retenciones sufridas» (ventas)**~~ | ✅ **COMPLETADO (2026-09-16)** — **backend** `src/withholding-reports/`: `GET /withholding-reports/suffered` (filtros por período, cliente, tipo y sucursal; filas por cobro con base, porcentaje, importe retenido, cuenta y documentos —criterio (a)—; **totales por tipo y por cliente** —criterio (b)—) y `GET /suffered/export?format=pdf\|excel` (certificado PDF + Excel de 4 hojas —criterio (c)—); **cuadre** —criterio (d)— contra el mayor de la cuenta del activo (propia del tipo `TYPE`, genérica del mapping `MAPPING` o `NONE` con aviso) a **precisión de centavo**, por (cuenta, período), con `criterion`/`dateAxis`/`scope` declarados y `warnings` accionables; **permiso** `withholding-taxes:view` y **filtro por sucursal** —criterio (e)—. **frontend**: página `/reports/withholding-suffered` con filtros, detalle paginado, totales, **panel de cuadre** (fuente de cada cuenta, debe/haber/neto, cuadrado/descuadrado, avisos y cobros anulados excluidos) y exportación PDF/Excel, con ruta de permiso propio, entrada de menú y tarjeta de informe. **Evidencia**: backend 22 unitarios + 6 E2E (informe ↔ asiento) y la suite completa (jest + `test:e2e`) en 0, más una **sonda independiente del revisor** contra la API y la BD de dev que cuadró **informe = BD = mayor** al centavo con limpieza verificada; frontend Karma **1647/1647** y E2E de UI **3 passed**, con los gates en 0. Contrato en `backend-erp/docs/withholding-reports.md`. Origen del riesgo: se declaraba como «riesgo de QA» y en realidad es **feature de producto** (ver `AUDIT.md` §8 T126, nota de reclasificación). |
| ~~Media~~ | ~~**F5.7 — Administración de tipos de retención (CRUD + pantalla LUNA)**~~ | ✅ **COMPLETADO (2026-09-16)** — el backend ya existía (`/withholding-taxes` GET/POST/PUT/DELETE + permisos); faltaba **toda la UI**, que es lo que se implementó: listado en `/withholding-taxes` con **búsqueda por código/nombre y filtro por estado** (cliente: la API devuelve un array plano), formulario LUNA con **alta/edición/modo ver** (código, nombre, **porcentaje 0–100 con 2 decimales** ⇄ `rate` como fracción `@Min(0) @Max(1)`, cuenta del pasivo y **cuenta del activo** de la retención sufrida, país y estado), ruta con `permissionGuard` por acción y `dirtyCheckGuard`, y entrada en el menú (**Administración → Tipos de Retención**). **Validaciones**: código **único por tenant** (409 con el mensaje del servidor + aviso local), **cuentas existentes y activas** (selector), porcentaje acotado y a 2 decimales, y **borrado bloqueado**: un tipo `isSystem` responde 400 (la UI no ofrece Eliminar) y con movimientos el backend **inactiva** devolviendo su mensaje, que la UI muestra tal cual. Evidencia: Karma **1628/1628** (27 nuevos) y E2E de UI **6 passed** (alta → API → edición → modo ver → borrado con limpieza verificada por API 404), más los gates del frontend en 0. **Nota**: los criterios «tipo» e «indicador de aplicación» de esta fila **no existen en el DTO**; no se inventaron campos. Origen del riesgo: se declaraba como «riesgo de QA» y en realidad es **feature de producto** (ver `AUDIT.md` §8 T126, nota de reclasificación). |
...
| ~~Media~~ | ~~**Cuentas partner local/extranjero sin resolver**~~ | Bifurcar CxC/CxP M/N vs M/E automáticamente. | ✅ **HECHO (T68, 2026-09-12):** `AccountDeterminationService` usa las variantes `receivableAccountIdLocal/Foreign` y `payableAccountIdLocal/Foreign` del socio según **moneda del documento** ≠ moneda base, con **moneda por defecto del socio** y **país del socio** (vs `Tenant.countryCode`, comparación normalizada nombre↔ISO) como refuerzo; la cuenta genérica sigue siendo el fallback. Verificado con E2E real (4 casos). |
| ~~Baja~~ | ~~**Nivel ITEM no lee el maestro del artículo**~~ | `Item.salesCreditAccountId` y similares existen en schema y forms pero la determinación nivel ITEM solo leía la matriz artículo-almacén. | ✅ **HECHO (T67, 2026-09-12):** `_walkItemHierarchy` cae al **maestro del artículo** cuando la matriz artículo-almacén no tiene la cuenta (y antes del fallback al AccountMapping global); el mensaje de error menciona ambas fuentes. 4 tests actualizados/nuevos. |
...
| Baja | **Descuento por pronto pago / retenciones en ventas** | Evaluar cuando el negocio lo pida. | ⚠️ **CORREGIDO (2026-09-16):** el **descuento por pronto pago** ya existe (entry type `EARLY_PAYMENT_DISCOUNT` + `earlyPaymentDiscountAmount` en cobros y pagos). Y las **retenciones en ventas SÍ existen desde T91** (2026-09-13): el **cobro con retención sufrida** ya contabiliza —el importe retenido se debita en la cuenta del activo del tipo (`WithholdingTaxType.receivableAccountId`) o en la genérica del mapping `WITHHOLDING_TAX_RECEIVABLE`, y el `IncomingPayment` guarda `withholdingAmount` + `withholdingTaxTypeId`—; esta fila decía lo contrario porque se escribió **antes** de que T91 cerrara ese defecto (antes rompía el guardado con un 500 de «Asiento desbalanceado»). Lo que falta no es el asiento sino su **reporte y certificado**: es **F5.6** (fila de arriba). |
...
*Última actualización: 2026-09-09 (G5 ✅; F5.7 Query Manager ✅ T21–T23/T40; D1–D5 ✅ del plan de gaps T33–T41; backlog de features consolidado abajo. Gaps de producto y deuda: ver `docs/plans/plan-gaps-deuda-2026-09.md`)*
...
**G2 el 2026-09-18** y **G3 se aprobó el 2026-09-19 y quedó **completo el 2026-09-20 con sus siete fases
implementadas** (maestros del ciclo productivo, receta multinivel con aviso de
faltantes, la **orden de producción** con snapshot de receta y ruta, estados
reversibles y costo previsto congelado, la **emisión para producción** que carga
los componentes al **WIP** de la orden, el **recibo para producción** que ingresa
el PT, los subproductos y la merma absorbiendo ese WIP, el **consumo de recursos**
que carga las horas y los servicios al mismo WIP y el **cierre** que liquida el WIP a
...
|---|---|---|
| **G3 — Módulo de Producción (órdenes, emisión/recibo, WIP)** | plan-gaps 2026-09 (grupo 1) | ✅ **Resuelto (2026-09-20)**: las **siete fases** implementadas y verificadas, plan aprobado por el usuario: plan `docs/plans/plan-g3-produccion.md` y estudio comparativo `docs/reference/PRODUCCION_ERP_COMPARATIVA.md` (cómo lo resuelven SAP B1, Odoo 19 y Dynamics 365, verificado contra documentación pública). Alcance propuesto: maestros de **centro de trabajo / recurso / componente de costo** (con las cuentas contables parametrizables que pidió el usuario), **BOM multinivel** con merma y operaciones, **orden de producción** con explosión de BOM y aviso de faltantes, **emisión para producción** (Dr WIP · Cr Inventario), **recibo de producción** (Dr PT/Subproducto/Mermas · Cr WIP) con coproducto y desperdicio, **consumo de recursos** (parte de horas con el snapshot de los componentes de costo y `Dr WIP / Cr cuenta del componente`, que además acumula el tiempo real de la operación) y **cierre con desviación** que deja el **WIP en 0** con su reapertura, los reportes de costo/WIP/desviaciones **en pantalla con exportación CSV** y el detector R16 (flujo SAP B1, con estados reversibles). Decisiones aprobadas por el usuario: D1 órdenes con explosión de BOM y aviso de faltantes (**sin MRP ni APS**), D2 **costo real por orden con liquidación al cierre** (sin costo estándar), D3 recurso con componentes de costo y **máquina = recurso `MACHINE` enlazado al Activo Fijo**, D4 escribir la referencia y el plan **antes** de codificar. Reutiliza el motor contable por jerarquía, el kardex, las series, las dimensiones/proyectos, la guarda de período (T151) y el motivo de anulación (T153); **`AssemblyOrder` no se migra** (conserva su contabilidad sin WIP). Límites declarados: sin MRP/APS, sin nómina (la mano de obra entra por componentes de costo a una tarifa), sin costeo estándar ni centro de costo en la línea de asiento (se resuelve por proyecto/dimensiones/regla de distribución). **Cierre de los límites declarados (2026-09-20, T158–T162)**: la orden publica el **mapa de sus documentos** (detalle con emisiones y recibos, pestaña «Documentos» con contadores, filtro **por orden** en los dos listados y **quién** cerró/reabrió/anuló), una operación **sin tiempo estándar** ya no deja la orden incerrable, el **proyecto** del documento llega al mayor y la orden captura las **cinco dimensiones**, los selectores de orden **buscan en el servidor** (sin el tope de 200) y el recorrido E2E de UI termina con las **dos operaciones** hechas, **subproducto y merma** ejercitados y la orden **cerrada** con el WIP en cero |
| **G4 — Orden de producción editable (componentes, operaciones, recurso y fechas)** | petición del usuario (pregunta por la orden de ensamblaje y por el flujo de SAP B1) | ✅ **Resuelto (2026-09-21)**: la orden de producción deja de ser un snapshot de solo lectura y se puede **editar su receta y su ruta** dentro del documento, como en SAP B1. **F1** componentes (quitar, agregar a mano y ajustar cantidad/merma; el `bomLineId` distingue «de la receta» de «manual»; botón **Traer receta** con la explosión multinivel y sus faltantes); **F2** operaciones (agregar, quitar, cambiar centro de trabajo, recurso y tiempos, con el costo recalculado); **F3** el **recurso se declara por operación** y los partes de horas solo usan los declarados (la máquina es un recurso `MACHINE`); **F4** **fecha de inicio de fabricación** en la cabecera, el detalle y el listado. Se edita en `DRAFT` y `PLANNED` (en planificada el costo previsto se recalcula y se vuelve a congelar). **La orden de ensamblaje queda declarada documento legacy de kits** (armar/desarmar): sus componentes son la receta del kit y no se editan en el documento, decisión escrita también en la excepción del gate `audit:line-accounts`. Límites declarados: sin MRP/APS (editar la receta en la orden no planifica compras) y las líneas manuales no se escriben en el maestro. Plan: `docs/plans/plan-g4-orden-produccion-editable.md`; evidencia en AUDIT **T166** (183 suites/2187 tests, E2E 31/251, Karma 1991, `e2e:visual` 53/53 sin regenerar y, de paso, el gate de dinero del backend devuelto a 0) |
| **Cuentas contables editables en las líneas (frente nuevo, 2026-09-21)** | petición del usuario (T163) | ✅ **Resuelto (2026-09-21): las cinco fases implementadas y verificadas**. La cuenta se **ve** en la línea, se **propone** desde la determinación contable (la misma que usa el motor) y se puede **cambiar a mano**; si se deja vacía, el motor resuelve por jerarquía como siempre. **P1** Revalorización (cuentas de aumento y contrapartida por línea), **P2** inventario (entradas, salidas y ajustes; el **traspaso** queda como excepción declarada: sus dos patas son la cuenta de inventario de cada almacén y el motor ignoraba el valor), **P3** comerciales (facturas de venta y compra, sus F. Reserva —que además no persistían la cuenta—, NC y devoluciones de venta y compra, entregas y recepciones, con la columna **visible** y la propuesta del `entryType` de cada familia), **P4** producción (emisión y recibo, con el WIP por la orden y las mermas por `lineType`), ensamblaje (API) y Precio de Entrega, y **P5** los dos gates `audit:line-accounts` con sus pasos de CI. Cierra además un **defecto medido**: el `acctCode` capturado no se validaba en ninguna familia comercial, así que una cuenta **agrupadora** se contabilizaba (400 atómico desde ahora, en **43 altas**). Límites declarados: los **partes de horas** no capturan cuenta (viene del maestro del recurso) y la **orden de ensamblaje no tiene grilla de líneas editable**, así que la captura por componente espera una superficie nueva. Plan: `docs/plans/plan-cuentas-contables-editables.md`; evidencia en AUDIT **T163**, **T164** y **T165** |
| **Centros de costo, dimensiones y normas de reparto (frente nuevo, 2026-09-21)** | petición del usuario | 🟡 **C0 resuelto (2026-09-21), C1–C4 pendientes de ejecución**: hoy los **5 ejes** (`DimensionConfig`) nombran las dimensiones, el **centro de costo es un valor de un eje** (`CostCenter.dimensionNumber`, como SAP B1) y las **normas de reparto** (`DistributionRule` + líneas por porcentaje) **ya se aplican al asiento contable manual** (expanden la línea en una por centro de costo con el importe prorrateado, escribiendo el código del centro en la dimensión del eje); lo que falta es (a) validar el valor del eje contra el maestro, (b) aplicar el reparto a los asientos **generados por documentos** y (c) los **informes** por centro de costo y por norma. **Decisión del usuario**: opción **C** —reutilizar el eje sin columna nueva, validar contra el maestro y extender el reparto a los documentos— con la norma de reparto **capturada por línea de forma opcional**, el eje de centros de costo declarado en la configuración de dimensiones y los informes mostrando el importe original de la línea y el reparto efectivo. Plan con las tres opciones y la decisión escrita en §0: `docs/plans/plan-centros-costo-y-reparto.md` |
| **G2 — Revalorización de artículos (Inventory Revaluation)** | plan-gaps 2026-09 (grupo 1) | ✅ **Resuelto (2026-09-18, T150)**: documento del módulo de **Inventario** que asigna el **costo resultante** de la existencia de un artículo en un almacén —por **precio unitario nuevo** (`PRICE`) o por **importe total nuevo** (`TOTAL`)—, actualiza `Stock.avgCost` (6 decimales), deja el **kardex con cantidad 0** (`StockMovement.valueDelta`, que además corrige el valorizado corrido) y contabiliza `Dr Inventario · Cr Revalorización` si el valor sube o `Dr Contrapartida · Cr Inventario` si baja (las dos cuentas que los maestros ya exponían en los 4 niveles); anular exige motivo y fecha de contabilización, revierte el asiento (T149) y restituye el costo. Serie `REV`, permisos `stock-revaluations:view|create|cancel`, detector R13 y frontend completo (listado + formulario con cálculo en vivo). Límites declarados: no reexpresa el COGS ya vendido, artículo + almacén (sin lote/serie) y moneda base. Ver `docs/plans/plan-g2-revalorizacion.md` y AUDIT **T150** (y **T152**: los dos defectos de interfaz que su E2E de UI destapó —el alta no se podía guardar y el listado no pintaba sus acciones, este último también en el Precio de Entrega desde G1—, corregidos) |
...
| **Lógica de licencias sobre los roles** (plan/subscripción → roles/features habilitadas) | ROADMAP 3.6 / Fase 8.1 | ✅ Infraestructura + **matriz de producto aprobada** (2026-09-09, T46): matriz `2026-09-09.3` con módulos core como piso común (ambos planes), capacidades del grupo 2 con destino por plan (SIN/CRM/divisa en ambos; SAP, nómina avanzada y localización solo DEDICATED; `planned` para las no implementadas), **volumetría D3** (SHARED 15 usuarios/5 almacenes/3 POS/BD compartida; DEDICATED sin límite y BD propia) + `planIncludes`/`planLimit` + `GET /billing/capabilities`. **D3 con enforcement (2026-09-09)**: `PlanLimitsService.assertCanAdd` bloquea con 403 el alta de usuarios/almacenes/terminales POS cuando el cupo está completo (solo el alta: nada existente se bloquea, D1) y `GET /billing/limits` expone consumo vs cupo. Sin gating de módulos actuales (D1 no regresión). Propuesta en `docs/plans/propuesta-matriz-licencias-2026-09.md`. Ver AUDIT T46 |
...
| **Densidad visual** (valores `px` crudos en scss de `shared/` + 28 tablas HTML crudas en páginas) | plan-gaps D5 (T41) | ✅ **Resuelto (2026-09-09, T48, Fase 2 cerrada)**: audit estático **307 → 0 hallazgos** con la convención "Comfortable/Compacta = look actual; solo Espaciosa relaja" aplicada a POS, Query Manager, 13 selectores `shared/`, tablas crudas (8 archivos + `payment-term-installments-preview` + `.modal-table` global) y 51 páginas más (230 conversiones + 7 `density-ok` justificados); gate CI `audit:density:ci` y verificación dinámica Compacta vs Espaciosa con **0 problemas** (`e2e:visual` 52/52). **Cierre del frente (2026-09-09)**: nueva auditoría de calidad de bloques `audit:density:vars` (+ fixer `audit:density:fix`) → **0 hallazgos** (eliminó 66 vars huérfanas y fusionó 17 bloques duplicados, corrigió 3 referencias cíclicas `--det-gap`/`--rep-opt-gap` que anulaban el espaciado en densidad Normal); **28 tablas crudas clasificadas** por familia con su fuente de densidad, las 2 de `item-detail` con regla propia 1px→4px, manifiesto dinámico 152→163 URLs (rutas `/reports/*`) y spec `e2e/density-raw-tables.spec.ts` con datos reales (**1px → 4px** verificado con el kit del seed). Gate visual: `e2e:visual` **52/52** tras resolver T53 (tipografías self-hosted). Ver AUDIT T48 |
| **`::ng-deep`** | plan-gaps D5 (T41) | ✅ Auditoría/cierre (2026-09-09, T49): N=60 menciones (31 archivos) → M=9 reglas activas justificadas con marcador; informe en `docs/reference/ng-deep-audit-2026-09.md`. Migración de los 9 a customization points LUNA queda como tracking del plan visual v2 Fase 7. Ver AUDIT T49 |
| **Gate visual `e2e:visual`** (deriva de altura en formularios de documentos/pagos) | AUDIT T48/T51 (QA E2E) | ✅ Resuelto (2026-09-09, T53): la deriva (subconjunto rotativo de formularios, +2px cuando la tipografía no cargaba) venía de las tipografías por CDN (Google Fonts), no del código. Fix: `scripts/fetch-webfonts.mjs` + `src/styles/_fonts.scss` self-hosted (Inter, JetBrains Mono) en `public/assets/fonts/`, Material Icons eliminado por no usarse, y espera de fuentes en `gotoForm`; 52 baselines regenerados y `e2e:visual` **52/52 en dos corridas consecutivas**. Ver AUDIT T53 |
