...
| Media | **F5.6 — Reporte y certificado de «retenciones sufridas» (ventas)** — **backend ✅ CERRADO (2026-09-16); UI en curso** | **Backend hecho y verificado**: módulo `src/withholding-reports/` con **`GET /withholding-reports/suffered`** (filtros por período, cliente, tipo y sucursal; filas por cobro con base, porcentaje, importe retenido y documentos del cobro y de la factura —criterio (a)—, **totales por tipo y por cliente** —criterio (b)—) y **`/suffered/export?format=pdf\|excel`** con certificado PDF y Excel de 4 hojas —criterio (c)—; **cuadre** —criterio (d)— contra el mayor de la cuenta del activo (la propia del tipo `TYPE` o la genérica del mapping `MAPPING`, declarando también `NONE`) a **precisión de centavo**, con desglose por (cuenta, período), alcance declarado (`ACCOUNT_MOVEMENT` / `REPORT_DOCUMENTS`) y avisos accionables; **permiso** `withholding-taxes:view` y **filtro por sucursal** —criterio (e)—. Evidencia: 22 tests unitarios + **6 E2E** (informe ↔ asiento, incluido el caso RC-IVA que cae a la cuenta genérica), `tsc`/`build`/`lint` en 0, el **revisor** corrió además la suite completa (jest + `test:e2e` 14 suites) con exit 0 y **midió el cuadre por su cuenta** contra la API y la BD de dev (informe = BD = mayor en el centavo, con limpieza verificada). Contrato campo por campo en `backend-erp/docs/withholding-reports.md`. **Falta la pantalla** (página LUNA con filtros, detalle, totales y panel de cuadre + exportación y su E2E), que es el frente en curso. |
