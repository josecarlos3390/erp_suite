...
| Media | **F5.7 — Administración de tipos de retención (CRUD + pantalla LUNA)** | Los tipos de retención (RC-IVA, IUE, IT…) se configuran **por seed o por API** (`/withholding-taxes` ya existe con GET/POST/PUT/DELETE); el frontend sólo los **consume** (cobros/pagos, ficha de partner, factura de compra). Alcance y **criterio de aceptación**: (a) listado con filtro por estado y búsqueda por código/nombre; (b) formulario (código, nombre, tipo, porcentaje, cuenta contable, vigencia, indicador de aplicación); (c) validaciones: código único por tenant, porcentaje 0–100 con dos decimales, cuenta contable existente y activa, borrado bloqueado si hay movimientos; (d) permisos por acción y respeto del aislamiento por tenant; (e) Karma del formulario + E2E de UI del alta/edición, y el seed actual queda como datos iniciales. Origen del riesgo: se declaraba como «riesgo de QA» y en realidad es **feature de producto** (ver `AUDIT.md` §8 T126, nota de reclasificación). |
