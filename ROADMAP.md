...
| **Densidad visual** (valores `px` crudos en scss de `shared/` + 28 tablas HTML crudas en páginas) | plan-gaps D5 (T41) | ✅ Infraestructura/gate (2026-09-09, T48): fix de 4 alias `--fs-*` inexistentes (uom-groups), migración a tokens del editor nuevo y baseline regenerado con `npm run audit:density:ci` (307 conocidos + 28 tablas; falla solo ante hallazgos NUEVOS). Migración completa de densidad de controles bespoke (POS/QM/reportes/shared) y de las tablas crudas sigue como backlog del plan visual v2 Fase 2. Ver AUDIT T48 |
