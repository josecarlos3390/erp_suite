...
| Entrega → Factura / F. Reserva | `POST /sale-invoices/from-delivery/:deliveryOrderId`, `/sale-reserve-invoices/from-delivery/:deliveryOrderId`, ambos `/from-multi-delivery` | `SaleInvoiceItem.deliveryOrderItemId` + `base*` (DELIVERY_ORDER) + `target*` en la línea de entrega + `SaleInvoice.deliveryOrderId` (**sólo origen único**); `DeliveryOrderItem.invoicedQty ↑` en confirm / ↓ en cancel | API entrega: `pendingInvoiceQty` **por línea**, `hasPendingToInvoice = items.some(pending > 0)`, `reservedQty` (FRV ligada), `hasLinkedReserveInvoice`; menú «Copiar a» | Σ **facturas activas** ligadas a la línea (T96/T97) | **R1** (relación), **R2** (vínculo inverso), **R3** (`invoicedQty` vs Σ facturas), **R4** (cabecera). **`isReserve` (decisión de contrato, 2026-09-18)**: el **default del API** al copiar de una entrega es **`'Y'`** (F. Reserva, igual que «desde cotización»; «desde pedido» es `'N'`) porque una factura nacida de una entrega **no debe mover inventario** —la mercancía ya salió con la entrega—; el **formulario manda `'N'`** explícito, así que la misma operación produce F. Reserva por API y factura normal por pantalla (divergencia declarada, sin efecto en stock). En **ambos** casos el stock no se mueve: las líneas con `deliveryOrderItemId` saltan el par COGS ↔ Inventario en `sales.journal-builder` («guard anti doble COGS»). Fijado por `test/sale-invoice-from-delivery-default.e2e-spec.ts` |
...
**Fecha de la reversa (límite declarado, T147)**: la reversa se fecha con el
**«hoy» del tenant** (`resolveDocumentDate(undefined)`, `accounting-engine.service`
≈L1932), no con la fecha del documento original. Es lo correcto frente a un período
**cerrado** (una anulación debe postear en un período abierto), pero implica que si
el documento pertenece a **otro día/período** del tenant, su original queda en el
período del documento y su espejo en el de hoy: el cuadre «informe ↔ mayor» de
`withholding-reports` para *ese* período conserva el original y **no cierra** (el
informe ya lo avisa con `anulado(s) con retención`). Medido con el tercer escenario
de la sonda de T146 y fijado por el test «límite declarado (T147)» de
`test/withholding-reports.e2e-spec.ts`. Las dos salidas posibles —fechar la reversa
en el período del original cuando esté abierto, o netear el par en el cuadre— son
**decisión de producto** pendiente (AUDIT T147).
