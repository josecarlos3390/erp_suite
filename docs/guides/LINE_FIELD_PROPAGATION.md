...
| 8 | Drafts de ventas y compras no devolvían `uomId`/`acctCode`/`dimension3-5`/`customFields` por ítem → el form destino mostraba la UoM vacía | Draft | Ventas: `sales-quotations.service.ts` (`convertToOrderDraft`), `delivery-orders.service.ts` (`getDraft`, `getDraftFromQuotation`, `getDraftMultiQuotation`, `getDraftMultiOrder`, `getDraftFromReserveInvoice`, `getDraftMultiReserveInvoice`), `sales-orders.service.ts` (`getDraft`, `createFromQuotation`). Compras: `purchase-quotations.service.ts` (`convertToOrderDraft`), `purchase-orders.service.ts` (`getDraft`, `getDraftMultiQuotation`), `purchase-receipts.service.ts` (`getDraftMultiOrder`, `getDraftMultiQuotation`, `getDraftMultiReserveInvoice`), `purchase-invoices.service.ts` (`getDraftMultiReceipt`, `getDraftMultiQuotation`, `getDraftMultiOrder`) | ✅ **RESUELTO (2026-08-24)**: los 16 drafts agregan `uomId: origen.uomId ?? item.salesUomId/purchaseUomId ?? item.inventoryUomId ?? null` (ventas usa `salesUomId`, compras `purchaseUomId`), `acctCode`, `dimension1-5`, `customFields` (y `projectCode` donde faltaba). Verificado por API en ambos lados: draft cotización→pedido lleva uomId → pedido creado con la UoM del origen. Los creates ya tenían fallback, por eso el documento final salía bien aunque el form lo mostraba vacío |
| 9 | Devoluciones (venta y compra) perdían `projectCode`/`dimension1-5`: `SalesReturnItem`/`PurchaseReturnItem` no tenían `projectCode`/`dimension1`/`dimension2` en el schema y el create no mapeaba NINGUNA dimensión (d3-5 existían como columnas pero no se persistían) | Schema + Create | `prisma/schema.prisma`, `prisma/manual/20260824_add_return_line_dimensions.sql`, `sales-returns.service.ts`, `purchase-returns.service.ts` | ✅ **RESUELTO (2026-08-24)**: columnas agregadas (schema + SQL manual aplicado), y los creates mapean `projectCode`/`dimension1-5` desde la línea del payload (`line.projectCode ?? null` etc.). Los DTOs ya heredaban los campos de `BaseLineItemDto` y los forms ya los enviaban — solo faltaba schema + create. Verificado por API: devolución creada con pc/d1-5/customFields → GET devuelve los valores |
