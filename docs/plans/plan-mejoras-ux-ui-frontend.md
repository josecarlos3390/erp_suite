...
- Tokens de altura: migrados `luna-modal-close` (sm), `luna-tabs--md` (md) y `skeleton-button` (sm/md/lg) en `edafa59`, y en `e5225794` se creó el tier **xs=28px** (`--size-control-xs`) que formaliza el patrón de botones icon-only en celdas densas, triggers compactos y tabs base (8 controles migrados). **✅ CERRADO (2026-09-16, T133):** los **2 micro-botones de 26px** de `batch-selector`/`serial-selector` pasaron a `var(--size-control-xs)` (y se retiraron sus variables de densidad huérfanas); las **~80 alturas restantes** del censo (2026-08-17) son decorativas (avatares, iconos, badges — sin token que las represente) o estructurales (filas de `luna-data-table` con densidad propia, contenedores de modales) y **no son accionables** sin inventar tokens de un solo uso; las 15 del POS viven en su scope. Clasificación completa en `plan-consistencia-visual-v2` Fase 2.
- Auditoría visual de POS (`pos.component.scss`, 1,951 líneas — scope separado). **Cubierta desde T84/T86/T87** (CSS muerto eliminado, clases acotadas con `:where(app-pos)`, gate `npm run audit:pos-scope`).
