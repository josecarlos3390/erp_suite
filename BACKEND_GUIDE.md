...
| 2 | **Totales persistidos** (journal builders, facturas, FRV, devoluciones/NC) | 🔄 **en curso**: **redondeo manual cerrado (R2b = 0**, con 3 justificaciones de porcentaje**)** con `scripts/migrate-round-money.mjs` y la aritmética ya migrada en `document-totals.util`, `payment-term.util`, `rc-iva`, `iue`, `delivery-orders`, `price-lists`, `price-resolver`, `purchase-invoices`, `sale-reserve-invoices`, `sale-invoices`, `reports`, `accounts`, `partners` y los restos de la corrección del detector (`document-drafts`, `fixed-assets`, `discount-cost.util`). **Frente abierto: 71** (R2a **63** + R2c **8**), por criticidad: `sales-debit-notes` (8 R2a), `purchase-credit-notes` (7), `bank-reconciliation` (6), `sales.journal-builder` (6), `purchase-debit-notes` (6), `purchase-orders` (5), `drafts.journal-builder` (1 + 3 R2c), `sales-orders` (4), `sales-credit-notes`/`purchase-requests` (3 cada uno), `items` (2 + 1 R2c), `incoming-payments` (1 + 2 R2c), `purchases.journal-builder` (2), `pos-sessions` (1), `banks` (1) y los pequeños |
