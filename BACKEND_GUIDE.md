...
| 13 | El detector reconoce el dinero **por el nombre** del identificador (o por el del argumento de `Number(...)`): un importe cuyo identificador no diga nada (`const v = Number(row[2])`, `Number(x)`, un índice de arreglo) queda **fuera** de las trece reglas, y las cantidades/tasas/porcentajes se excluyen también por nombre | ⚠️ **declarado como límite de diseño (2026-09-15)**, corregible sólo **por tipos**, no con una regex sobre texto. La **fase 5** cerró su parte accionable: el contrato interno (`JournalEntryLineData` y las columnas de la doble expresión) ya **declara `Decimal`** el dinero, así que en esa cadena no puede haber aritmética en flotante (`tsc` lo prueba). Lo que sigue fuera del detector es el dinero que **sólo** se puede reconocer leyendo tipos: un valor `number` sin nombre de dinero y sin contrato tipado (p. ej. un cálculo intermedio local). La regla práctica de §8.4 sigue siendo la misma: **el dinero se llama por su nombre y se declara `Decimal`** |
