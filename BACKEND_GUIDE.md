...
| 2 | **Totales persistidos** (journal builders, facturas, FRV, devoluciones/NC) | 🔄 **en curso**: **redondeo manual cerrado (R2b = 0)** con `scripts/migrate-round-money.mjs`, y **aritmética ya migrada** en `document-totals.util`, `payment-term.util`, `rc-iva.service.ts`, `iue.service.ts`, `delivery-orders.service.ts`, `price-lists.service.ts`, `price-resolver.util.ts`, `purchase-invoices.service.ts`, `sale-reserve-invoices.service.ts` y `sale-invoices.service.ts`. Con el **detector ya corregido** (R2c y R2a v2), el inventario es **96** (R2a 67 + R2c 29). Frente abierto por criticidad: `journal-entries` (10), `payment-term.util.ts` (9), `reports` (8), `partners` (6), `bank-reconciliation` (5), `sales-debit-notes` (4) |
...
**Punto de continuación**: `journal-entries.service.ts` (**10** R2c, ahora el mayor), con el
paso 2 hecho en serio. Sigue el orden de criticidad `payment-term.util.ts` (9), `reports` (8),
`partners` (6), `bank-reconciliation` (5). *(Ya migrados: `document-totals.util`,
...
`sale-reserve-invoices.service.ts` y `sale-invoices.service.ts`.)* **Cuando el camino
