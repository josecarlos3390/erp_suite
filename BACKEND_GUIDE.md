...
| 4 | Reportes y lecturas | ⏳ |
| 5 | **El canal `*InBaseCurrency`** (deuda de **tipado**, medida el 2026-09-15) | ⏳ **medido, sin migrar**: las columnas son `Decimal(14,2)` (`prisma/schema.prisma`), pero el **contrato** las pasa como `number` en varios puntos —`JournalEntryLineData.debitInBaseCurrency`/`creditInBaseCurrency` son `number \| null` y los DTO/informes que las consumen también—: **188 menciones en 19 archivos** (sin contar specs), encabezadas por `journal-entries` (32), `document-totals.util` (22), `purchase-invoices` (20) y los pedidos (16 + 16). No es deuda de **aritmética** en cada sitio (la convierte a `Decimal` quien persiste), sino de **tipo**: mientras el contrato diga `number`, media cadena puede operar en flotante sin que ninguna regla lo vea. Se declara aquí para que el hueco tenga número; el arreglo es un cambio de **tipo del contrato** (una decisión de diseño, no una migración mecánica), y el detector no puede vigilarlo hasta que exista |
| 6 | **Las guardas de dinero del frontend** (medidas el 2026-09-15) | ✅ **CERRADA en su parte medida (2026-09-15)**: el frontend estrena utilidad canónica **espejo** de la del backend (`erp-frontend/src/app/shared/money/money.util.ts`: centavos enteros, `moneyEquals` a **0 centavos** para los cuadres y `isSettled`/`exceedsBy` con `SETTLEMENT_TOLERANCE_CENTS` = **1 centavo** para las liquidaciones, con los **mismos nombres** para que las dos capas digan lo mismo) y se migraron **21 guardas**: `incoming-payments-form` (13: 9 cuadres a 0 centavos y 4 tolerancias a 1 centavo), `outgoing-payments-form` (5), `journal-entries-form` (2: partida doble y preview, ambos a **0 centavos**) y `document-form.base` (1: el reparto del descuento de cabecera, 0 centavos). Evidencia: utilidad con **20 tests** que discriminan contra la implementación de épsilon (`2,3 − 2,29 = 0,00999…`), Karma **1589/1589** y eslint 0/0. **Declarado, fuera de esa parte**: `incoming-payments-form` 503/533 y `shared/installment-allocation-table` 106/219/224 (`remaining <= 0.001` en la proyección de cuotas — sustituirlo **no** es mecánico: el centavo de `isSettled` dejaría `remaining` en −0,01 y la cuota siguiente recibiría un importe **negativo**; requiere decisión de negocio + dejar `remaining` a 0), `partner-account-statement` 627 (`maxReconcileAmount + 0.001`) y `commercial-document-form.base` 683 (costo unitario a 6 decimales). Los `0.01` que **no** son dinero (porcentajes que suman 100, `step="0.01"`, cantidades) quedan como están |
...
| 13 | El detector reconoce el dinero **por el nombre** del identificador (o por el del argumento de `Number(...)`): un importe cuyo identificador no diga nada (`const v = Number(row[2])`, `Number(x)`, un índice de arreglo) queda **fuera** de las trece reglas, y las cantidades/tasas/porcentajes se excluyen también por nombre | ⚠️ **declarado como límite de diseño (2026-09-15), no corregible con este detector**: distinguirlo exige **tipos** (que el dinero se declare `Decimal` en el contrato) o un AST con información de tipos, no una regex sobre texto. Es la razón por la que la **fase 5** (el canal `*InBaseCurrency`, tipado `number`) importa: cerrar ese tipo cierra a la vez este límite. Mientras tanto, la regla práctica sigue siendo la de §8.4: **el dinero se llama por su nombre y se pasa como `Decimal`** |
