...
| T25 | Backend / QA integral (maratón e2e de uso real) | No existia una prueba automatizada que simulase USO MASIVO del ERP (un mes completo de operacion + cierre contable) encadenando compras/ventas/inventario/lotes/series/servicios/descuentos/pagos/NC y validando cuadraturas al cierre. | RESUELTO (2026-09-07): nueva suite test/usage-marathon.e2e-spec.ts (11 fases, API real + tenant aislado en erp_test): Fase 0 crea gestion FY-2026 con periodos mensuales por API y verifica validate-date; Fases 1-2 compras PO->recepcion->factura con descuento y LOTE + pago proveedor (asientos PURCHASE_RECEIPT/PURCHASE_INVOICE/OUTGOING_PAYMENT balanceados, stock, stockBatch, CxP saldada); Fase 3 ventas SQ->SO->DO->SI con descuento (asiento DELIVERY_ORDER con COGS + SALE_INVOICE con CxC/ingresos/IVA y desglose SALES_DISCOUNT) + SERVICIO sin COGS + SERIAL (serie creada por API, recepcion y venta la marcan SOLD, reventa rechazada 400); Fase 4 movimientos de almacen (entrada/salida/traspaso entre almacenes, asientos STOCK_ENTRY/EXIT/TRANSFER); Fase 5 NC de venta + cobro cliente (CxC saldada); Fase 6 cierre: ningun asiento desbalanceado, closing-report con ecuacion.ok=true, close() deja LOCKED y rechaza asientos del mes (409) pero permite otro mes abierto. 11/11 verde. HUELLAS ABIERTAS (documentadas, NO corregidas a ciegas - normativa contable BO): 5 asserts E2E desactualizados vs el motor (commits posteriores al test: 8fe13f5/7f79073 desglose 87/13 del descuento y 44a88f0 NC no revierte IT): discount-propagation 4 tests esperan "netting sin SALES_DISCOUNT" (el motor hoy desglosa 87/13 segun perfil BO+BOLIVIA_SIN) y returns-and-credit-notes test 6 espera reversa del IT en NC (el motor 44a88f0 ya no la hace, regla normativa). Requieren decision de negocio/actualizacion de asserts. Ver AUDIT T24 y backend-erp/CHANGELOG.md | P1 |
