...
43. **Baterías QA 18-27 (tributario, concurrencia, períodos, tenant 2, cierre anual) + hardening de seguridad + convención NC venta BO** (`qa + backend + frontend / go-live`) — `✅ Parcial: 26/27 validadas en verde; 18-25 pendientes de corrida integral`
...
49. **Corrida integral de baterías 01-28 + E2E de la UI del POS (go-live 2026-08-24)** (`qa + backend + frontend / go-live`)
   ...
   - **Pendiente:** push a GitHub de los fixes (credenciales intermitentes en este entorno) y re-corrida k6 `large` con rate limit elevado.
...
50. **Ensamblajes → AccountingEngine (deuda AGENTS §5.7 item 6 cerrada) + FIXME 1 resuelto + k6 prod** (`backend + frontend / go-live 2026-08-24`)
   ...
   - **k6 large contra build de producción (start:prod):** kardex-report **100%** (27.694 checks) y `sale-invoice-load` **86.3%** (13.7% de fallos vs 16.7% en dev — el build compilado mejora pero el techo de escritura sostenida a 25 VUs persiste). **Diagnóstico corregido (2026-08-24):** los tenants de load-test SÍ tienen plan de cuentas, mappings e IVA (factura manual 201 verificada) y el ítem del escenario es de servicio (no consume stock) — los fallos restantes de `sale-invoice-load`/`multitenant-isolation` son **contención de la fila del partner**: los 25 VUs facturan al MISMO cliente y `updatePartnerBalance` serializa en su fila (row-lock, integridad correcta) → la cola crece y las requests superan el timeout (p90 7.8s). El escenario maximiza la contención de un solo partner; en producción con partners distintos se paraleliza. `bulk-import-concurrency` falla por las expectativas de status del escenario (safe-mode import). **Acción pendiente:** re-diseñar los escenarios k6 con partner/ítem por VU (seed multi-partner en el tenant de load) para medir el throughput real sin contención artificial.
