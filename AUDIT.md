...
| **Backend / contabilidad IVA descuento 87/13** | IVA del descuento iba a la cuenta cruzada (compras→débito, ventas→crédito); ahora cada lado reduce SU impuesto (compras→crédito, ventas→débito) — Form 200 limpio | ✅ Resuelto (2026-08-13) |
...
21. **Fix: IVA del descuento 87/13 iba a la cuenta cruzada (compras→débito, ventas→crédito)** (`backend / accounting`) — `✅ Resuelto` (2026-08-13)
   - **Síntoma (validado con normativa):** la FRC/NC de compra acreditaba/debitaba el 13% del descuento contra **IVA — Débito Fiscal** (2.1.2.01.001), y la FV/NC/ND/devolución de venta lo hacía contra **IVA — Crédito Fiscal** (1.1.6.01.001) — la cuenta *cruzada* de cada lado. El neto de IVA por pagar era correcto, pero las **casillas del Form 200 se contaminaban**: `debitoFiscal` incluía un componente de compras y `creditoFiscal` aparecía bruto (195 en vez de 175.50), porque el reporte (`reports.service.ts`) lee los saldos de cuenta 2.1.2.01.001/1.1.6.01.001.
   - **Normativa:** Ley 843, Art. 8, inciso b) — los descuentos/devoluciones del período se **deducen del crédito fiscal** del comprador. El débito fiscal solo se reduce del lado del **vendedor que emite la NC** (o en NC de ventas). Por tanto, cada lado reduce **SU propio impuesto**: compras → IVA Crédito Fiscal (Cr en FRC, Dr en NC); ventas → IVA Débito Fiscal (Dr en FV/FRV, Cr en NC/ND/devolución).
   - **Fix:** `purchases.journal-builder.ts` (invoice + NC: `TAX_OUTPUT` → `TAX_INPUT`) y `sales.journal-builder.ts` (FV + NC×2 + ND + devolución×2: `TAX_INPUT` → `TAX_OUTPUT`), incluidas descripciones y comentarios (`account-mappings.util.ts`). Sin migración de datos (los asientos viejos posteados quedan como estaban).
   - **Cobertura:** 6 tests actualizados en `accounting-engine.service.spec.ts` (FRC 87/13 → IVA Crédito 700 C 15.60; FV/FRV BO → IVA Débito 300 D 46.28/18.20; STANDARD sin desglose). Backend 133 suites / 1317 tests. Verificado en vivo por API: FRC → Cr IVA Crédito 19.50, NC → Dr IVA Crédito 19.50, FV → Dr IVA Débito 19.50 (asientos balanceados, Form 200 limpio). Documentado en `ACCOUNTING_ENTRIES_GUIDE.md` §6/§7.
