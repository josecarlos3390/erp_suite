...
| T10 | UX | Alturas decorativas sin token + Fase 7 del plan visual (`::ng-deep`) | 🟡 **En curso (2026-09-05, ondas web/móvil)** — inventario: 117 menciones en 32 archivos → **58 reglas vivas en 27 archivos** (resto: comentarios de historial). Onda 1: `::ng-deep` eliminados en `luna-input` (propio pseudo-elemento). Alturas 28/38/44/48/52px: excepciones estructurales intencionales (sin token 1:1) documentadas. Siguientes ondas por familias: filtros (consolidar en `_lists.scss` con CSS vars), labels de forms, overlays/menús | P3 |
