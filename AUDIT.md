...
| **QA visual F4 (frontend vs batería) — 1 bug real encontrado** | QA visual de listados y forms (FV/FRV/NC/ND/FRC) contra los números validados por la batería: dashboard (37.224 ventas / 60.270 compras / 24.670 CxC / 56.782,47 CxP), FVE-000355 (280.14/41.86/322), FRV-000346 (desc 1.810, 14.172,30/2.117,70/16.290), NCR-000166 (3.288,60/491,40/3.780), FRC-000409 (7.308/1.092/8.400) — todo coincide. **Bug real: columna Sucursal '—' en listados de FV y FRV** — el `findAll` no incluía la relación `branch` (el frontend bindea `branch.name`; las 19 FVs tenían branchId=1). Fix: `branch: true` en ambos findAll, verificado por API y en el UI. NC/ND/FRC/FCP sí mostraban la sucursal (includes compartidos). **Pendiente: la página de Notas Débito (`/sales-debit-notes`) renderiza el main vacío** (sin toolbar ni tabla, reproducible en varias navegaciones) — requiere diagnóstico con consola del browser (posible bug del componente o artefacto del dev server HMR). Observaciones P3: 'Precio Unit.' muestra el precio payload (350) vs 'Precio Total' resuelto (322) en FV manual; totales sin formato de miles en listado de F. Reserva Compra; saldo pendiente no visible en FRV | ✅ Resuelto (2026-08-17, fix e631882) — ND pendiente de diagnóstico |
