...
| T10 | UX | Alturas decorativas sin token + Fase 7 del plan visual (`::ng-deep`) | 🟡 **En curso (2026-09-05, ondas web/móvil)** — inventario: 117 menciones en 32 archivos → **58 reglas vivas**. Onda 1: `::ng-deep` en `luna-input`. Onda 2 (familia de filtros): default global de `.filter-search` desktop+móvil en `_layout.scss` y selectores de `.filter-field` en `_lists.scss` (+`app-price-list-selector`) → −20 reglas en udf/item-boms/item-price-histories/exchange-rates/assembly-orders. **Quedan 11 reglas vivas** (extensión de tokens LUNA en marcha: ya `--luna-label-*`, `--luna-form-field-gap`, `--filter-bar-*`, `--filter-search-*`, `--luna-action-icon-size`; eliminadas las action-bars móviles duplicadas de sales/purchase quotations — el global `_forms.scss` ya las cubre. Residual: triggers de selectores (NC venta), pierce interno legítimo por ng-content (luna-empty-state svg, partner-selector clear, mixin `_inventory-form-lines`), cell de luna-input (price-list-form), audit-logs y stock-valuation). Alturas 28/38/44/48/52px: excepciones estructurales intencionales documentadas | P3 |
