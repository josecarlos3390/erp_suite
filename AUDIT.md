...
65. **Despliegue en producción: backend en Railway + frontend en Vercel conectados (go-live 2026-08-25)** (`deploy / infra`)
   ...
   - **Drift de schema (hallazgo importante):** el schema tiene columnas/tipos que las migraciones (hasta 2026-07-20) y los SQL manuales NO cubren (ej. `User.defaultPosTerminalId`, enums `CostCenterStatus`, `AssemblyOrderStatus`...) — el proyecto usa `db push` en dev y el drift nunca se capturó. Una BD fresca queda sin esas columnas → el seed falla. **Fix aplicado:** `prisma migrate diff --from-url <bd> --to-schema-datamodel` filtrado (solo aditivo + conversión a enum; se excluyeron los DROP de `_manual_migrations`/`_BulkImportLock` e índices) → aplicado a la BD de Railway. **Deuda:** el drift debe capturarse en el repo (re-baseline de migraciones o bootstrap SQL) para que futuras BDs frescas desplieguen sin pasos manuales.
