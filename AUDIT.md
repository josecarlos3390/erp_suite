...
23. **Costeo NETO (NIC 2): el descuento de cabecera se prorratea al costo del inventario** (`backend + frontend / costeo`) — `✅ Resuelto` (2026-08-13)
   ...
   - **Cobertura:** nuevo spec `discount-cost.util.spec.ts` (6 tests); Backend 133 suites / 1317 tests + Karma 63/63 (line-array + forms PO/FRC/cotización). Verificado en vivo por API: cotización → PO → recepción → FRC → NC con costo **234.90**/ud, avgCost **234.90**, asientos netos sin Descuentos y GRIR cerrado. Documentado en `ACCOUNTING_ENTRIES_GUIDE.md` §6/§7.
