...
50. **Ensamblajes → AccountingEngine (deuda AGENTS §5.7 item 6 cerrada) + FIXME 1 resuelto + k6 prod** (`backend + frontend / go-live 2026-08-24`)
   ...
   - **k6 large contra build de producción (start:prod):** kardex-report **100%** (27.694 checks) y `sale-invoice-load` **86.3%** (13.7% de fallos vs 16.7% en dev — el build compilado mejora pero el techo de escritura sostenida a 25 VUs persiste). `bulk-import-concurrency` y `multitenant-isolation` fallan por **setup de los escenarios** (los tenants de load-test no tienen el plan de cuentas completo → la creación de facturas falla por determinación de cuentas; no es bug de la app — el aislamiento pasa). Pendiente: ajustar el seed de `createTestData` para los escenarios multitenant/bulk.
