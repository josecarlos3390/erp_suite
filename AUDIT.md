...
59. **Fix: la UoM (y otros campos de línea estándar) no viajaban en los drafts de los flujos de ventas** (`backend / propagación, 2026-08-24`)
   ...
   - **Fix (7 drafts):** `convertToOrderDraft` (cotización→pedido), `getDraft`/`getDraftMultiOrder` (pedido→entrega), `getDraftFromQuotation`/`getDraftMultiQuotation` (cotización→entrega directa/multi), `getDraftFromReserveInvoice`/`getDraftMultiReserveInvoice` (FRV→entrega) en delivery-orders, y `getDraft` + `createFromQuotation` en sales-orders. Regla: `uomId: origen.uomId ?? item.salesUomId ?? item.inventoryUomId ?? null`, `acctCode`, `dimension2-5`, `customFields` (con `projectCode`/`dimension1` donde faltaban). Las interfaces `Draft*ItemDO` extendidas.
   - **Verificado:** `tsc` + lint 0/0 + build; probe por API: draft cotización→pedido ahora lleva uomId y el pedido creado conserva la UoM de la cotización; specs de sales-quotations/sales-orders/delivery-orders en verde; suite backend completa en verde. Documentado en `docs/guides/LINE_FIELD_PROPAGATION.md` (gap 8 resuelto).
